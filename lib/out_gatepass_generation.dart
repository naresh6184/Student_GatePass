...
class OutGatePassPage extends StatefulWidget {
  const OutGatePassPage({super.key});

  @override
  _OutGatePassPageState createState() => _OutGatePassPageState();
}

class _OutGatePassPageState extends State<OutGatePassPage> {
...
Future<void> _selectDate(BuildContext context, bool isOutDate) async {
  ...
  Future<void> generateGatePass(
    ...
    String gatepassType = "OUT STATION"; // Example value, can be dynamic
    int doaaApproval = 0; // Indicates pending by default
    int chiefWardenApproval = 0;
...
String branch = userSnapshot['branch'];
    ...
    String userBranch = userData['branch'] ?? '';

    String hodApprovalField = '';

    // Based on the branch, set the correct HOD approval field
    if (userBranch == 'Information Technology' || userBranch == 'Computer Science & Engineering' || userBranch == 'Computer Science & Design' || userBranch =='Integrated Dual Degree (CSE+AI)') {
      hodApprovalField = 'hodCSApproval';
    }
    else if (userBranch == 'Chemical Engineering') {
      hodApprovalField = 'hodCEApproval';
    }
    else if (userBranch == 'Petroleum Engineering' || userBranch =='Integrated Dual Degree (Petroleum)') {
      hodApprovalField = 'hodPEApproval';
    }
    else if (userBranch == 'Mathematics & Computing') {
      hodApprovalField = 'hodM&CApproval';
    }
    else if (userBranch == 'Electronics Engineering' || userBranch == 'Electronics Engineering Major in E-Vehicle') {
      hodApprovalField = 'hodEEApproval';
    }
    else {
      // Fallback for branches not defined, you can add more branches here if needed
      hodApprovalField = 'hodApproval'; // Default HOD approval field
    }
    ...
    Map<String, dynamic> gatePassData = {
      ...
      'doaaApproval': doaaApproval,
      'chiefWardenApproval': chiefWardenApproval,
      hodApprovalField: 0,  // Add dynamic HOD approval field with initial value
    };
